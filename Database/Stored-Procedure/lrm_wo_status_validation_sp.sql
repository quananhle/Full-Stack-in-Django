...
DECLARE 
	...
	BEGIN
		...
		IF transtype = 'PRINT-PRESHIP' THEN

			t_pallet_id := v_value;

			IF t_pallet_id = '' THEN
				RAISE EXCEPTION 'Pallet ID cannot be blank.' USING HINT = 'Custom Error';
			END IF; 
		
			IF NOT EXISTS(SELECT 1 FROM shp_pallet sp WHERE pallet_id = t_pallet_id) THEN
		         RAISE EXCEPTION 'Pallet ID % Not Existed.', t_pallet_id USING HINT = 'Custom Error';
		    END IF;
